...
if (!empty($lrn)) {
    ...
    if ($checkResult && $checkResult->num_rows > 0) {
        ...
        if ($row['count'] > 0) {
            ...
            header("Location: view_teacher_recordd.php?lrn=" . urlencode($lrn) . "&status=lrn_not_found");
            exit;
        }
    } else {
        // If the learners query failed, redirect with an error message
        header("Location: view_teacher_recordd.php?status=query_failed");
        exit;
    }
} else {
    // If no LRN is provided, redirect with an error message
    header("Location: view_teacher_recordd.php?status=missing_lrn");
    exit;
}
...
header("Location: view_teacher_recordd.php?lrn=" . urlencode($lrn) . "&status=success");
exit;
?>
